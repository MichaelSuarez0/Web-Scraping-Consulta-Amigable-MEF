...
class ConsultaAmigable():
    def __init__(self, ruta: str, timeout: int = 100, headless=False):
        ...
        self.URL_ANUAL = "https://apps5.mineco.gob.pe/transparencia/Navegador/"
    ...
    async def initialize_driver(self):
        ...
        self.page.set_default_timeout(6_000)
    ...
    async def navigate_to_url(self, mensual: bool = False):
        """
        Navega a la URL especificada utilizando el driver proporcionado.
        """
        if not mensual:
            await self.page.goto(self.URL_ANUAL)
        else:
    ...
    async def select_dropdown_option(self, element_id, option_text):
        ...
        """
        try:
            await self.page.locator(f'select[id={element_id}]').select_option(str(option_text))
        except TimeoutError:
            logging.error("Rayos")
    ...
    async def extract_data_by_year(self, year, route_name, table_headers):
        ...
        await self.select_dropdown_option(a_config.GLOBAL_SELECTORS["year_dropdown"], year)
...
async def main():
    ...
    driver = await session.initialize_driver()
    todos_los_datos = []
    table_headers = []

    try:
        await session.navigate_to_url()
